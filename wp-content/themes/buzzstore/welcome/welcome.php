				...
				if( is_admin() && ('themes.php' == $pagenow) && (isset($_GET['activated'])) ) {
					...
					echo '<div class="notice notice-success is-dismissible buzzstore-activation-notice">';
						...
						echo '<h1>';
							...
							printf( esc_html__( 'Welcome to %s', 'buzzstore' ), esc_html( $theme_data->Name ) );
						echo '</h1>';

						echo '<p>';
							/* translators: %1$s: theme name, %2$s link */
							printf( __( 'Thank you for choosing %1$s! To fully take advantage of the best our theme can offer please make sure you visit our <a href="%2$s">Welcome page</a>', 'buzzstore' ), esc_html( $theme_data->Name ), esc_url( admin_url( 'themes.php?page=buzzstore-welcome' ) ) );
						echo '</p>';

						echo '<p><a href="'. esc_url( admin_url( 'themes.php?page=buzzstore-welcome' ) ) .'" class="button button-primary button-hero">';
							/* translators: %s theme name */
							printf( esc_html__( 'Get started with %s', 'buzzstore' ), esc_html( $theme_data->Name ) );
						echo '</a></p>';
					...
					<div class="abt-promo-wrap clearfix">
						<div class="abt-theme-wrap">
							<h1><?php printf( // WPCS: XSS OK.
								...
								esc_html__( 'Welcome to %1$s - Version %2$s', 'buzzstore' ), $this->theme_name, $this->theme_version ); ?></h1>
							<div class="about-text"><?php echo esc_html__( 'Buzzstore is a clean, beautiful and fully customizable responsive Free WooCommerce WordPress theme. This theme is packed with lots of exciting features that enhance the eCommerce experience. This theme is fully compatible with WooCommerce and some other external plugins like YITH Wishlist, YITH Quick View, WOOF Products Filter for WooCommerce, Jetpack, Contact Form 7 and many more plugins. This theme packs many premium features and several custom widgets which helps to make your online store, eCommerce Websites.', 'buzzstore' ); ?></div>
						</div>

						<div class="promo-banner-wrap">
							<p><?php esc_html_e('Upgrade for $55', 'buzzstore'); ?></p>
							<a href="<?php echo esc_url('https://www.sparklewpthemes.com/wordpress-themes/buzzstorepro'); ?>" target="_blank" class="button button-primary upgrade-btn"><?php echo esc_html__('Upgrade Now', 'buzzstore'); ?></a>
							<a class="promo-offer-text" href="<?php echo esc_url('https://www.sparklewpthemes.com/wordpress-themes/buzzstorepro'); ?>" target="_blank"><?php echo esc_html__('Unlock all the possibitlies with Buzzstore Pro.', 'buzzstore'); ?></a>
						</div>
					</div>
			...
			public function buzzstore_welcome_styles_and_scripts($hook) {
				if( 'appearance_page_buzzstore-welcome' == $hook ){
					$importer_params = array(
						'installing_text' => esc_html__('Installing Importer Plugin', 'buzzstore'),
